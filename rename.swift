...
func replaceName(at path: String, name: String) throws {
    let workspaceContents = try String(contentsOfFile: path)
    try workspaceContents.replacingOccurrences(of: "ProjectTemplate", with: name).write(toFile: path, atomically: true, encoding: .utf8)
}
